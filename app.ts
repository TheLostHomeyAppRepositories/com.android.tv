...
class AndroidTV extends Homey.App implements LoggerInterface {
  ...
  private async registerFlowCardListeners(): Promise<void> {
    this.homey.flow.getActionCard('open_link').registerRunListener(this.onFlowActionOpenLink.bind(this));
  ...
  async onFlowActionOpenApplication({
    device,
    app,
  }: {
    device: RemoteDevice;
  ...
  async onFlowActionLongPressKey({
    device,
    option,
    seconds,
  }: {
  ...
  async onFlowKeyAutocomplete(
    query: string,
    { device }: { device: RemoteDevice },
  ): Promise<FlowCard.ArgumentAutocompleteResults> {
