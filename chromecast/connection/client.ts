...
class Client extends EventEmitter<ClientEvents> {
  ...
  private onPacket = (buf: Uint8Array): void => {
    ...
    this.emit(
      'message',
      message.namespace,
      message.payloadType === PayloadType.BINARY ? message.payloadBinary : message.payloadUtf8,
      message.sourceId,
  ...
  private logMessage(log: string, message: ICastMessage): void {
    this.debug(
      ...
      message.namespace,
      ', data=',
      message.payloadType === PayloadType.BINARY ? util.inspect(message.payloadBinary) : message.payloadUtf8,
    );
  }
}
